resource "azurerm_kubernetes_cluster" "aks_cluster" {
   ...
   resource_group_name = azurerm_resource_group.rg.name     #required
    ...
    depends_on = [
      ...
      azurerm_application_security_group.web_asg,
      azurerm_application_security_group.was_asg
    ]
    linux_profile {
...
resource "azurerm_kubernetes_cluster_node_pool" "web_pool" {
    ...
    node_count = 1
    enable_auto_scaling = true
    vnet_subnet_id = azurerm_subnet.web_subnet.id
    node_network_profile {
        allowed_host_ports {
            port_start = 80
            port_end = 80
...
resource "azurerm_kubernetes_cluster_node_pool" "was_pool" {
    ...
    node_count = 1
    enable_auto_scaling = true
    vnet_subnet_id = azurerm_subnet.was_subnet.id
    node_network_profile {
        allowed_host_ports {
            port_start = 80
            port_end = 80
        ...
        application_security_group_ids = [ 
            azurerm_application_security_group.was_asg.id
        ]
 	}
    tags = {
